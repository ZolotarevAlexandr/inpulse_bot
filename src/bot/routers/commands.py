...
async def statistics_cmd(message: Message):
    ...
    text = (
        ...
        f"  Shown (7d): {s.recommendations_7d}  ·  (30d): {s.recommendations_30d}\n"
        f"  Avg per user: {s.avg_recommendations_per_user}\n"
        f"  Acceptance rate: {_pct(s.acceptance_rate)}\n"
        f"  Rejection rate: {_pct(s.rejection_rate)}\n"
        f"  Completion rate: {_pct(s.completion_rate)}\n"
        f"  Avg response time: {f'{s.avg_response_minutes} min' if s.avg_response_minutes is not None else '—'}\n"
        "\n"
        "⚙️ <b>System</b>\n"
        f"  Uptime: {uptime}\n"
